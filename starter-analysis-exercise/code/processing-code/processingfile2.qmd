...
```{r}
d1 <- rawdata %>% dplyr::filter( Height != "sixty" ) %>% #delete the observation containing "sixty"
                  dplyr::mutate(Height = as.numeric(Height))#convert the column into numeric 
skimr::skim(d1) # function provides a summary of the structure and contents of a datase
hist(d1$Height)
head(d1)
```
...
```{r}
d3 <- d2 %>%  
  dplyr::filter(Weight != 7000) %>% #delete observation with weight 7000 
  tidyr::drop_na() # drop the N/A observation 
...
```{r}
d3$Gender <- as.factor(d3$Gender) # convert to factor/ categorical   
skimr::skim(d3)
head(d3)
```
...
```{r}
d4 <- d3 %>% 
  dplyr::filter( !(Gender %in% c("NA","N")) ) %>% # Removes rows where Gender is either "NA" or "N"
  droplevels()
skimr::skim(d4)
```

Instead of using age as numeric, we can categorize the age

```{r}
d4 <- d4 %>%
  mutate(age_cat = ifelse(Age <= 30, 0, 
                  ifelse(Age >= 31 & Age <= 40, 1, 
                  ifelse(Age >= 41 & Age <= 50, 2, 
                  ifelse(Age > 50, 3, NA)))))
head(d4)
```

In Hair color, there is an observation having hair color O. A
...
```{r}
save_data_locatio <- here::here("starter-analysis-exercise","data","processed-data","processeddata2.rds")
saveRDS(processeddata2, file = save_data_locatio)
```
